...
<head>
  ...
  <script>
    ...
    const API_KEY = 'gssjDzHae%2B6Wsz0FDw9lftPAZUvrYK7aagq%2FI0OWl5SIgd7IWKHIJ7yMRNsm2NDEh5jDOILlxXGhcICVvQnb4w%3D%3D';
    ...
    async function getCrimeDate() {
    	...
    	*/
    	...
    	str += res.data.map((item, i) =>
    		...
    		+ '<td>' + item.살인 + '건</td>'
    		+ '<td>' + item.폭력 + '건</td></tr>'
    	).join('');					// map형식의 연속된 자료는 배열처럼 저장된다. 이때 기본 구분자가 ','인데, 이것을 join()함수를 이용하여 공백으로 바꿔준다.
    	str += '</table>';
    	$("#demo").html(str);
    }
